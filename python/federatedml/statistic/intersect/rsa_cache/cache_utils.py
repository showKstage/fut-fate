...
def get_rsa_of_current_version(host_party_id, id_type, encrypt_type, tag, timeout=60):
    ...
    if namespace is None or version is None:
        LOGGER.info('host_get_current_verison return None, partyid={}, id_type={}, encrypt_type={}, tag={}.'.format(
            host_party_id, \
...
def store_rsa(host_party_id, id_type, encrypt_type, tag, namespace, version, rsa):
    ...
    with DB.connection_context():
        LOGGER.info(
            'store rsa and out table info, partyid={}, id_type={}, encrypt_type={}, namespace={}, version={}.'.format(
                host_party_id, \
...
def clean_all_rsa(host_party_id, id_type, encrypt_type, tag='Za'):
    ...
    with DB.connection_context():
        LOGGER.info(
            'clean rsa and out table info, partyid={}, id_type={}, encrypt_type={}, tag={}.'.format(host_party_id, \
                                                                                                    id_type,
                                                                                                    encrypt_type, tag))
