...
class HeteroSVDTransferVariable(BaseTransferVariables):
    def __init__(self, flowid=0):
        ...
        self.dh_ciphertext_bc = self._create_variable(name='dh_ciphertext_bc', src=['arbiter'], dst=['guest', 'host'])#密文
        ...
        self.dh_pubkey = self._create_variable(name='dh_pubkey', src=['arbiter'], dst=['guest', 'host'])#公钥
        self.guest_average_rate = self._create_variable(name='guest_average_rate', src=['guest'], dst=['host'])#μ
        self.guest_loss = self._create_variable(name='guest_loss', src=['guest'], dst=['arbiter'])#客户损失
        ...
        self.guest_uuid = self._create_variable(name='guest_uuid', src=['guest'], dst=['arbiter'])#随机id
