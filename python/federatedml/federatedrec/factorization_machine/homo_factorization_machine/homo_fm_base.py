...
from federatedml.federatedrec.factorization_machine.base_factorization_machine import BaseFactorizationMachine
...
from federatedml.param.factorization_machine_param import HomoFactorizationParam
...
class HomoFMBase(BaseFactorizationMachine):
    ...
    def compute_wx_plus_fm(self, data_instances, model_weights):
        """calculate w*x + (v*x)^2 - (v^2)*(x^2)
            ...
            model_weights.w_: feature_size
            model_weights.embed_: feature_size * embed_size
        """
        ...
        def fm_func(features, embed):
            ...
            re = np.sum(re, 0)  #在第一维方向上求和
            part1 = np.sum(np.power(re, 2))  #一维
        ...
        #计算wx+fm部分的公式
        #mapValues()
        wx = data_instances.mapValues(lambda v: np.dot(v.features, model_weights.w_) + model_weights.intercept_)#intercept截距Θ0
