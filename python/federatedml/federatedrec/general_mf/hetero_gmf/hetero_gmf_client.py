...
from federatedml.federatedrec.optim.sync import user_num_transfer_sync
from federatedml.federatedrec.general_mf.hetero_gmf.hetero_gmf_base import HeteroGMFBase
from federatedml.federatedrec.general_mf.hetero_gmf.backend import GMFModel
from federatedml.federatedrec.general_mf.hetero_gmf.gmf_data_convertor import GMFDataConverter
    ...
    def predict(self, data_inst):
       ...
       # LOGGER.info(f"data_inst type: {type(data_inst)}, size: {data_inst.count()}, table name: {data_inst.get_name()}")
        ...
        if self.flowid == 'validate':
            ...
            label_data = fate_session.parallelize(zip(keys, labels), include_key=True, partition=data_inst.partitions)
        else:
