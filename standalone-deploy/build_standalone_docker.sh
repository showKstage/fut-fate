...
#  fateboard_git_branch=`grep -A 3 '"fateboard"' .gitmodules | grep 'branch' | awk -F '= ' '{print $2}'`
  # replace git repo with hard code
  fateboard_git_url=https://gitee.com/chengjuzhen/FATE-Board.git
  fateboard_git_branch=v1.6.0
...
init() {
    ...
    docker build -t fate:latest-new .
#    docker run -d --name fate -p 8080:8080 fate:latest-new /bin/bash
