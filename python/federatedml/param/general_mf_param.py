...
class GMFParam(BaseParam):
    ...
    def __init__(self,
                 ...
                 #联合类型；Union[X, Y]意味着：要么是 X，要么就是 Y,参数一定是类型
                 batch_size=-1,
...
class HeteroGMFParam(GMFParam):
    ...
    def __init__(self,
                 optimizer={"optimizer": "SGD", "learning_rate": 0.01},      #SGD:随机梯度下降法
                 batch_size=-1, init_param=GMFInitParam(), max_iter=100,     #Batch Size定义：一次训练所选取的样本数
