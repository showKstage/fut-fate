...
from federatedml.federatedrec.optim.sync import user_ids_transfer_sync, average_rate_transfer_sync
from federatedml.federatedrec.svd.hetero_svd.hetero_svd_base import HeteroSVDBase
from federatedml.federatedrec.svd.hetero_svd.backend import KerasSeqDataConverter, SVDModel

LOGGER = log_utils.getLogger()

LOGGER.info('scc - run client')
...
class HeteroSVDClient(HeteroSVDBase):
    ...
    def join_average_rate(self, rates_table):#111
        # TODO: use map partition for parallelization 并行化操作
        rates = [float(_r) for _, _r in rates_table.collect()]
        # ratetable 就是 再base下的extractid取出来的userid itemid rate
        # LOGGER.info(f"rates len {len(rates)}, data {rates}")

        average_rate = np.average(rates) #received average rate [0.3333333333333333, 26712]
    ...
    def join_user_ids(self, user_ids_table):
        ...
        join_user_ids = user_ids_table.union(received_user_ids)  # 复制？
    ...
    def fit(self, data_instances, validate_data=None):
        ...
        while self.aggregator_iter < self.max_iter:
            ...
            unboxed = self._model.modify(modify_func)
            #LOGGER.info(f"WHW-client-unboxed-type:{type(unboxed)}")
            #LOGGER.info(f"WHW-client-unboxed:{unboxed}")

            # calc loss and check convergence
            if self._check_monitored_status(data, epoch_degree):
                user_Bias = unboxed['user_bias']
                user_Embedding = unboxed['user_embedding']
                LOGGER.info(f"WHW-client-User-type:{type(user_Bias)}")
                LOGGER.info(f"WHW-client-user_Bias:{user_Bias}")
                LOGGER.info(f"WHW-client-user_Embedding:{user_Embedding}")
    ...
    def predict(self, data_inst):
        user_ids_table, item_ids_table, rates_table = self.extract_ids(data_inst)
        rates = [float(_r) for _, _r in rates_table.collect()]
        # user_ids = sorted([_id for (_id, _) in user_ids_table.collect()])
        data = self.data_converter.convert(data_inst, self._model.user_ids, self._model.item_ids,self.batch_size)
        ...
        cnt = 0
        #这里找到了predict大于阈值的 将其分类为1 代表可以推荐的
        myid = []
        ...
        for i in range(len(rates)):
            if rates[i] == 0:
                myrateIndex.append(i)
        rateCnt = 0
        newTargetCustomers = []

        for x in zip(data.get_keys(), predict):
            if myrateIndex[rateCnt] != myid.index(x[0]):
                continue
            elif x[1][0] <= threshold:
                rateCnt += 1
            elif myrateIndex[rateCnt] == myid.index(x[0]) and x[1][0] > threshold: #找到id矩阵
                newTargetCustomers.append(int(x[0]))
                # LOGGER.warn(f"Init rate is 0; After rate is 1 id is {TOPid[cnt]} PREDICT IS {TOPpred[cnt]}")
                cnt += 1
                rateCnt += 1
            else:
                continue
        LOGGER.warn(f"Id of the new target customer is: {newTargetCustomers}")
        newName = ['customerId']
        transCsv = pd.DataFrame(columns=newName, data=newTargetCustomers)
        LOGGER.warn(f"transCsv is: {transCsv}")
        transCsv.to_csv('/fate/target1.csv',encoding='utf-8')
        pred_tbl = fate_session.parallelize(kv, include_key=True, partition=data_inst.partitions)
        return data_inst.join(pred_tbl, lambda d, pred: [float(d.features.get_data(2)), pred[0], pred[1], {"label": "target" if (float(d.features.get_data(2))==0) and (pred[0]==1) else "no target"}])
