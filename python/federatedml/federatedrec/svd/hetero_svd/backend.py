...
from federatedml.federatedrec.utils import zip_dir_as_bytes
from arch.api.utils import log_utils

LOGGER = log_utils.getLogger()

LOGGER.info('scc - run backend')
...
class ConstantLayer(Layer):
    ...
    def build(self, input_shape):
        # Create a weight variable for this layer. 设置系数
...
class SVDModel:
    def __init__(self, user_ids=None, item_ids=None, embedding_dim=None, mu=None):
        ...
        :param embedding_dim: embedding dimension 嵌入维数
        :param mu: average rate of training data 训练数据的平均速率
        """
        ...
        self._aggregate_weights = None#总权重
    ...
    def _build(self, lamda_u=0.0001, lamda_v=0.0001, optimizer='rmsprop',
        ...
        LOGGER.info(f"WHW-_build-user_InputLayer:{user_InputLayer}")
        LOGGER.info(f"WHW-_build-user_EmbeddingLayer:{user_EmbeddingLayer}")
        LOGGER.info(f"WHW-_build-user_BiasLayer:{user_BiasLayer}")

        LOGGER.info(f"WHW-_build-item_InputLayer:{item_InputLayer}")
        LOGGER.info(f"WHW-_build-item_EmbeddingLayer:{item_EmbeddingLayer}")
        LOGGER.info(f"WHW-_build-item_BiasLayer:{item_BiasLayer}")

        # rating prediction
        dotLayer = Dot(axes=-1, name='dot_layer')([user_EmbeddingLayer, item_EmbeddingLayer])
        LOGGER.info(f"WHW-_build-item-dotLayer1:{dotLayer}")

        # add mu, user bias and item bias
        dotLayer = ConstantLayer(mu=self.mu)(dotLayer)
        LOGGER.info(f"WHW-_build-item-dotLayer2:{dotLayer}")
        dotLayer = Add()([dotLayer, user_BiasLayer])
        dotLayer = Add()([dotLayer, item_BiasLayer])
        LOGGER.info(f"WHW-_build-item-dotLayer3:{dotLayer}")

        # create model
        self._model = Model(inputs=[user_InputLayer, item_InputLayer], outputs=[dotLayer])
        LOGGER.info(f"WHW-_build-self._model:{self._model}")
    ...
    def build_model(cls, user_ids, item_ids, embedding_dim, loss, optimizer, metrics, mu):
        ...
        LOGGER.info(f"WHW-build_model-model1:{model}")
        model._build(loss=loss, optimizer=optimizer, metrics=metrics)
        LOGGER.info(f"WHW-build_model-model2:{model}")
        return model
    ...
    def modify(self, func: typing.Callable[[Weights], Weights]) -> Weights:
        ...
        #LOGGER.info(f"WHW-modify-weights:{weights}")
        unboxed = self.set_model_weights(func(weights))
        return unboxed
    ...
    def set_model_weights(self, weights: Weights):
        ...
        #LOGGER.info(f"WHW-set_model_weights-unboxed:{unboxed}")
